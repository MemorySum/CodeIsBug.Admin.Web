<template>
    <div>
        ...
        <el-card style="float: left; width: 49%; height: 100%">
          <div slot="header" class="clearfix">
            <span>角色列表</span>
          </div>
          <div>
            <el-tree ref="roleTree" :data="this.RoleTreeData" node-key="RoleId" :props="this.defaultProps"
                     default-expand-all check-strictly highlight-current @node-click="GetMenuInfoByRoleId">
            </el-tree>
          </div>
        </el-card>

      <el-card style="float: right; width: 50%; height: 100%">
        <div slot="header" class="clearfix">
          <span>菜单列表</span>
          <el-button style="float: right" type="primary" size="small" @click="saveRoleMenuSubmit">保存授权</el-button>
        </div>
        <div>
          <el-tree ref="menuTree" :data="this.MenuTreeData" node-key="MenuId" show-checkbox :props="this.defaultProps"
                   default-expand-all>
          </el-tree>
        </div>
      </el-card>

  ...
  <div>
    ...
    <div>
      <el-card class="box-card" shadow="never">
        ...
        <div>
          <el-table :data="tableData" style="width: 100%;height: 100%;" border row-key="MenuId" border
            ...
            <el-table-column prop="IsDeleted" label="是否删除" min-width="120">
              <template slot-scope="scope">
                <el-tag :type="scope.row.IsDeleted === 0 ? 'primary' : 'danger'">
                  {{scope.row.IsDeleted=== 0 ? '否' : '是'}}
                </el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="Level" sortable label="层级" min-width="60">
            </el-table-column>
            <el-table-column prop="Sort" sortable label="排序" min-width="60">
            </el-table-column>
            <el-table-column prop="AddTime" sortable label="添加时间" min-width="120">
            </el-table-column>
            <el-table-column prop="ModifyTime" sortable label="最后修改时间" min-width="120">
            </el-table-column>
          ...
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-sizes="limits"
        ...
        <el-form :model="form" :rules="Rules" ref="addRuleForm">
          ...
          <el-form-item label="父级菜单名称" :label-width="formLabelWidth" prop="ParentId">
            <el-select v-model="form.ParentId" placeholder="请选择父级菜单" clearabl size="medium">
              ...
              <el-option v-for="item in menuOptions" :key="item.MenuId" :label="item.Name" :value="item.MenuId">
              </el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="是否删除" :label-width="formLabelWidth">
            <el-switch v-model="form.IsDeleted" active-color="#ff4949" inactive-color="#dcdfe6" active-value="1"
              inactive-value="0">
            </el-switch>
          </el-form-item>
      ...
      <el-dialog title="编辑菜单" :visible.sync="dialogFormEditVisible" center width="30%" @open="loadingEditData">
        <el-form :model="editForm" :rules="Rules" ref="editRuleFormRef">
          ...
          <el-form-item label="是否删除" :label-width="formLabelWidth">
            <el-switch v-model="editForm.IsDeleted" active-color="#ff4949" inactive-color="#dcdfe6" active-value="1"
              inactive-value="0">
            </el-switch>
          </el-form-item>
          <el-form-item label="层级" :label-width="formLabelWidth">
...
<script>
  export default {
    ...
    methods: {
      loadData() {
        this.$http.get('/api/Menu/GetMenus', {
      ...
      editSubmit() {
        this.$refs.editRuleFormRef.validate((isvalid) => {
          if (!isvalid) {
            ...
            this.$http.post('/api/Menu/UpdateMenu', this.editForm).then(res => {
              if (!res) {
      ...
      getAllFirstLevelMenu() {
        ...
        this.$http.get('/api/Menu/GetAllFirstLevelMenu').then(res => {
          if (!res) {
      ...
      edit(id) {
        ...
        this.getAllFirstLevelMenu()
        this.$http.get('/api/Menu/GetMenuInfo', {
          params: {
            menuGuid: id
          }
        }).then(res => {
          ...
          if (rtnData.Code !== 1) {
            ...
            this.editForm.Level = rtnData.Object.Level.toString()
            this.editForm.ParentId = rtnData.Object.ParentId.toString()
            this.editForm.IsDeleted = rtnData.Object.IsDeleted.toString()
      ...
      del(id) {
        ...
        this.$confirm('此操作将删除该菜单, 是否继续?', '提示', {
          ...
          this.$http.get('/api/Menu/DelMenu', {
            params: {
              menuGuid: id
            }
          }).then(res => {
